...
if(isset($_POST['delete'])){
   ...
   $delete_wishlist_item = $db->prepare("DELETE FROM `wishlist` WHERE id = ?");
   $delete_wishlist_item->execute([$wishlist_id]);
}

if(isset($_GET['delete_all'])){
   $delete_wishlist_item = $db->prepare("DELETE FROM `wishlist` WHERE user_id = ?");
...
<section class="products">
   ...
   <?php
      $grand_total = 0;
      $select_wishlist = $db->prepare("SELECT * FROM `wishlist` WHERE user_id = ?");
   ...
   <form action="" method="post" class="box">
      ...
      <img src="msc/<?= $fetch_wishlist['image']; ?>" alt="">
